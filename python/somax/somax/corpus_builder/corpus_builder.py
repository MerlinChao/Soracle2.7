...
class SegmentationStatistics:
    def __init__(self, source: np.ndarray, sr: float, hop_length: int, onset_frames: np.ndarray,
        ...
        duration_times: np.ndarray = librosa.frames_to_time(
            duration_frames, sr=sr, hop_length=hop_length)
        ...
        self.discarded_ratio: float = 1 - \
            np.sum(duration_times) / \
            self.audio_file_length_s  # assumes audio length > 0
...
class ThreadedCorpusBuilder(multiprocessing.Process):
    def __init__(self,
        ...
        self.target = SimpleOscTarget(
            address=osc_address, port=send_port, ip=ip)
        self.kwargs = kwargs
    ...
    def run(self):
        ...
        self.target.send(ServerSendProtocol.BUILDING_STATUS,
                         ["init", self.builder_address])

        try:
            corpus: Corpus = CorpusBuilder().build(
                filepath=self.filepath, corpus_name=self.corpus_name, **self.kwargs)
            self.logger.debug(
                f"[build_corpus]: Successfully built '{corpus.name}' from file '{self.filepath}'.")
        except ValueError as e:  # TODO: Missing all exceptions from CorpusBuilder.build()
            self.logger.error(f"{str(e)}. No corpus was built")
            self.target.send(ServerSendProtocol.BUILDING_STATUS, [
                             "failed", self.builder_address])
            return
            ...
            self.logger.error(
                f"The file format of the provided file is not supported.")
            self.target.send(ServerSendProtocol.BUILDING_STATUS, [
                             "failed", self.builder_address])
            return
        ...
        if self.output_folder is not None:
            ...
            try:
                ...
                self.logger.info(
                    f"Corpus was successfully written to file '{output_filepath}'.")
                self.target.send(ServerSendProtocol.BUILDING_CORPUS_DONE, [
                                 output_filepath, self.builder_address])
        ...
        self.target.send(ServerSendProtocol.BUILDING_STATUS,
                         ["success", self.builder_address])


class CorpusBuilder:
    MIDI_FILE_EXTENSIONS = [".mid", ".midi"]
    AUDIO_FILE_EXTENSIONS = [".aif", ".aiff", ".wav",
                             ".flac", ".mp3"]  # .ogg doesn't work in python
    ...
    def build(self, filepath: str, corpus_name: Optional[str] = None, **kwargs) -> Corpus:
        ...
        if os.path.isdir(filepath):
            filepaths, content_type = self._folder_content(
                filepath)  # type: List[str], Optional[Type[Corpus]]
            if content_type == AudioCorpus:
                raise IOError(
                    "Building audio corpus from folder is not supported")
            name: str = corpus_name if corpus_name is not None else os.path.basename(
                filepath)
        else:
            content_type = self._parse_content_type(filepath)
            filepaths = [filepath]
            name = corpus_name if corpus_name is not None else os.path.splitext(
    ...
    def _folder_content(self, filepath: str) -> Tuple[List[str], Optional[Type[Corpus]]]:
        ...
        for file in sorted(os.listdir(filepath)):
            file_content_type: Optional[Type[Corpus]
                                        ] = self._parse_content_type(file)
            if file_content_type is None:
                ...
                raise IOError(
                    "Building corpus from mix of audio and midi files is not supported.")
    ...
    def _build_midi(self,
                    ...
                    spectrogram_filter: AbstractFilter = AbstractFilter.parse(
        ...
        self.logger.debug(
            f"[_build_midi]: ({0:.2f}) building MIDI corpus '{name}'...")
        ...
        bg_matrix: NoteMatrix = note_matrix.split_by_channel(
            background_channels)
        stft: Spectrogram = Spectrogram.from_midi(
            bg_matrix, spectrogram_filter, **kwargs)
        self.logger.debug(
            f"[_build_midi]: ({timer() - start_time:.2f}) MIDI-based STFT computed")
        ...
        events: List[MidiCorpusEvent] = self.slice_midi(
            note_matrix, build_parameters=build_parameters, **kwargs)

        self.logger.debug(
            f"[_build_midi]: ({timer() - start_time:.2f}) segmented MIDI into {len(events)} slices")

        # TODO: Folder support - should not use filepaths[0]
        metadata: MidiMetadata = MidiMetadata(
            filepath=filepaths[0], content_type=RelativeScheduling(), stft=stft)

        used_features: List[Type[AnalyzableFeature]] = []
        # type: Type[AnalyzableFeature]
        for feature in AnalyzableFeature.classes():
            try:
        ...
        self.logger.debug(
            f"[_build_midi]: ({timer() - start_time:.2f}) completed construction of MIDI corpus")
    ...
    def _build_audio(self,
        ...
        start_time: float = timer()
        self.logger.debug(
            f"[_build_audio]: ({0:.2f}) Building audio corpus '{name}'...")

        if len(filepaths) > 1:  # TODO Temporary: needed to support offline corpus building but not supported in UI
            self.logger.warning(
                "Loading multiple audio files is not supported yet in the UI")
        ...
        onset_frames, duration_frames, stats = self._slice_audio(audio_signal=y,
                                                                 ...
                                                                 **kwargs)
        onset_times: np.ndarray = librosa.frames_to_time(
        ...
        foreground_data: np.ndarray = self._parse_channels(
            y, channels=foreground_channels)
        background_data: np.ndarray = self._parse_channels(
            y, channels=background_channels)

        # TODO: Allow passing of stft parameters (win_length, n_fft, window function, ...)
        stft: Spectrogram = Spectrogram.from_audio(
            background_data, sample_rate=sr, hop_length=hop_length, **kwargs)
        ...
        self.logger.debug(
            f"[_build_audio]: ({timer() - start_time:.2f}) computed necessary metadata")
        ...
        self.logger.debug(
            f"[_build_audio]: ({timer() - start_time:.2f}) completed construction of audio corpus")
    ...
    def _slice_audio(self,
        ...
        y = self._parse_channels(
            audio_signal=audio_signal, channels=onset_channels)
    ...
    def _compute_slice_durations(y: np.ndarray, sr: float, hop_length: float, onsets: np.ndarray,
        ...
        if max_size_s is not None:
            max_size_frames = librosa.time_to_frames(
                max_size_s, sr=sr, hop_length=int(hop_length))
        ...
        if min_size_s is not None:
            min_size_frames = librosa.time_to_frames(
                min_size_s, sr=sr, hop_length=int(hop_length))
    ...
    def _slice_audio_by_interval(y: np.ndarray, sr: float, hop_length: int = 512,
        ...
        duration_samples: np.ndarray = interval_samples * \
            np.ones_like(onset_frames)
    ...
    def slice_midi(note_matrix: NoteMatrix, build_parameters: Dict[str, Any],
        ...
        events: List[MidiCorpusEvent] = [
            MidiCorpusEvent.incomplete(index, note_matrix.notes.iloc[0])]
        # TODO[OPTIMIZATION]: Using iterrows will be very slow for large matrices. Subject to optimization
        # type: pd.Series
        for i, note in note_matrix.notes.iloc[1:].iterrows():
            # Finalize previous and create a new CorpusEvent
            if note[Keys.ABS_ONSET] > events[-1].absolute_onset + slice_tolerance_ms:
                events[-1].set_duration(end=note[Keys.REL_ONSET],
                                        absolute_end=note[Keys.ABS_ONSET])
                index += 1
                corpus_event: MidiCorpusEvent = MidiCorpusEvent.incomplete(
                    index, note)
                corpus_event.append_notes(
                    events[-1].held_from(), events[-1].onset, events[-1].absolute_onset)
        ...
        # Finalize last slice
        final_note: Note = max(
            events[-1].notes, key=lambda n: n.onset + n.duration)
        note_end: float = events[-1].onset + \
            max(0.0, final_note.duration + final_note.onset)
    ...
    def add_missing_audio_features(self, corpus: AudioCorpus,
        ...
        all_features: List[Type[AnalyzableFeature]] = cast(
            List[Type[AnalyzableFeature]], AnalyzableFeature.classes())
        ...
        self.logger.info("Added features: " +
                         ", ".join([f.__name__ for f in added_features]))
        return corpus
