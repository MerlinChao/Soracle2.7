...
class OscAgent(Agent, AsyncioOscObject):
    ...
    def set_param_fo(self, parameters: dict[str,str]):
        self.player.set_param_fo(parameters)
    ...
    #    
    #    self.player.create_vmo_player( features_used, vmo_threshold, max_continuity, navigation_mode,window_size, taboo_length,
    #                                transpositions, fallback_mode, behaviour_when_influenced, weights,isguided, temperature)

    def create_vmo_player(self,*kwargs):
        print("agent create_vmo_player")
        self.player.create_vmo_player(*kwargs)

    def create_vmo_player_with_multi_seg(self,foldername , *kwargs):
        ...
        for feature_type in all_features:
            ...
            if os.path.isfile(filepath):
                corpuses[feature_type] = AudioCorpus.from_json(filepath)


        self.player.create_vmo_player_with_multi_seg(corpuses,*kwargs)
    

    def load_multi_seg(self, foldername: str, *kwargs):
        all_features = [YinDiscretePitch, OnsetChroma,  Mfcc, RMS,SpectralCentroid]
        corpuses = {}
        format = "pickle"
        for feature_type in all_features:
        ...
        self.player.load_multi_seg(corpuses,*kwargs)
    ...
    def visualisation_omax_v2(self, foldername: str):
        self.player.visualisation_omax_v2(foldername)

    def create_vmos(self) -> None:
        self.player.create_vmos()

    def save_oracle(self,path):
        try:
            self.player.save_vmo(path)
            self.logger.info(f"Oracle saved to {path}") 
        except (FileNotFoundError, IOError) as e:
            self.logger.error(f"Could not save oracle to {path}: {str(e)}")

    def load_oracle(self, path):
        try:
            self.player.load_vmo(path)
            self.logger.info(f"Oracle loaded from {path}")
        except (FileNotFoundError, IOError) as e:
            self.logger.error(f"Could not load oracle from {path}: {str(e)}")

    def send_match_statistics(self):
        #print("send_match_statistics")
        """ Gets the current state in each layer, not including the merged layer """
        external_dict, internal_dict, match = self.player.get_matches_statistics()
        total_external = sum(external_dict.values())
        total_internal = sum(internal_dict.values())
        match = "match" if match else "no_match" 
        self.target.send( ["matches_statistics","total"], total_external+total_internal)
        for name, count in external_dict.items():
            name = self.string_from_feature(name)
            self.target.send( ["matches_statistics","external", name], count)
        for name, count in internal_dict.items():
            name = self.string_from_feature(name)
            self.target.send( ["matches_statistics","internal", name], count)
        self.target.send( ["matches_statistics","match"], match)
