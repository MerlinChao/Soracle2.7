...
from somax.features.feature import FeatureUtils, AnalyzableFeature, CorpusFeature
...
class RMS(CorpusFeature):
    ...
    def decode(cls, trait_dict: Dict[str, Any]) -> 'CorpusFeature':
        return cls(value=trait_dict["RMS"])

    def encode(self) -> Dict[str, Any]:
        return {"RMS": self._value}

    def value(self) -> float:
        return self._value
    
    @classmethod
    def _analyze_audio(cls, events: List[CorpusEvent], metadata: AudioMetadata) -> List[CorpusEvent]:
        rms = librosa.feature.rms(y=metadata.foreground_data, frame_length=2048, hop_length=metadata.hop_length)
        for event in events:
            onset_frame = librosa.time_to_frames(event.onset, sr=metadata.sr, hop_length=metadata.hop_length)
            end_frame = librosa.time_to_frames(event.onset + event.duration, sr=metadata.sr, hop_length=metadata.hop_length)
            if end_frame - onset_frame == 0:
                mean_rms = float(rms[0, onset_frame])
            else:
                mean_rms = float(np.mean(rms[0, onset_frame:end_frame]))
            event.set_feature(cls(mean_rms))
