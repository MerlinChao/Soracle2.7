...
class FactorOracle(object):
    ...
    def is_recognized(self, context):
        """Check if the context is recognized by the Factor Oracle.
        
        Args:
            context: A sequence (same type as the oracle data).
        
        Returns:
            accepted (bool): Whether the sequence is recognized.
            final_state (int): State where the recognition stopped.
        """
        _next = 0
        for symbol in context:
            for target_state in self.trn[_next]:
                if self.data[target_state] == symbol:
                    _next = target_state
                    break
            else:
                return False, _next
...
def find_threshold_ir(input_data, r=(0, 1, 0.1), flag='a', suffix_method='inc',
    ...
    for t in thresholds:
        ...
        nb_clusters.append(tmp_oracle.num_clusters())
        if entropy:
    ...
    if entropy:
        return ir_thresh_pairs[0], pairs_return,nb_clusters, h0_vec, h1_vec
    else:
        return ir_thresh_pairs[0], pairs_return, nb_clusters
